...
namespace TAGDigitalStudios.AstroAlgo
{
	...
	{
		/// <summary>An encapsalated date to work with since DateTime is sealed.</summary>
		...
		public DateTime GetEquinoxSolstice(ES season)
		{
			return Julian2Date( EquinoxSolstice(this.now.Year, season) );
		}
		...
		public static DateTime AshWednesday(DateTime e)
		{
		...
		{
			if (y < 1583)
				throw new AAException("Input year must be in the Gregorian Calendar (greater than 1582).");
			...
			return new DateTime(y, n, p+1, 0, 0, 0);
		}
		...
		public static DateTime Julian2Date(double JD)
		{
			...
			if ( E < 14 )
				...
				throw new AAException("Illegal month calculated.");
			...
			if ( month > 2 )
				...
				throw new AAException("Illegal year calculated.");
			...
			else
			{
				...
				else
				{
					if ( d.Day < 5 )
						...
						throw new AAException("Input day falls between 10/5/1582 and 10/14/1582, which does not exist in the Gregorian Calendar.");
				} // end middle else
			} // end outer else
		...
		public static void ApparentSolarCoordinates(double JD, out double alpha, out double delta)
		{
			double	T;		/* Julian Centuries */
			...
			alpha = Revolution(Math.Atan2(Math.Cos(ep0*Deg2Radian) * Math.Sin(lamda*Deg2Radian), Math.Cos(lamda*Deg2Radian)) * Radian2Deg);
			delta = Math.Asin(Math.Sin(ep0*Deg2Radian) * Math.Sin(lamda*Deg2Radian)) * Radian2Deg;
		}
		...
		public static void AzimuthAltitude(double jd, double alpha, double delta, double L, double phi, ref double A, ref double h)
		{
			...
			/* calculate azimuth */
			A = Math.Atan2( Math.Sin(H * Deg2Radian), (Math.Cos(H * Deg2Radian) * Math.Sin(phi * Deg2Radian) - Math.Tan(delta* Deg2Radian) * Math.Cos(phi * Deg2Radian)) ) * Radian2Deg;
		
			/* calculate altitude */
			h = Math.Asin( Math.Sin(phi * Deg2Radian) * Math.Sin(delta * Deg2Radian) + Math.Cos(phi * Deg2Radian) * Math.Cos(delta * Deg2Radian) * Math.Cos(H * Deg2Radian) ) * Radian2Deg;
		}
		...
		{
			double	deltaPsi = 0;		/* nutation in longitude */
			double	deltaEpsilon = 0;	/* nutation of obliquity */
			double	epsilon = 0;		/* true obliquity of the ecliptic */
			double	epsilonNull = 0;	/* mean obliquity of the ecliptic */

            // convert day to Julian Centuries
            double T = JulianCenturies(jd);
		
            // calculate nutation and obliquity values
			Nutation(T, out deltaPsi, out deltaEpsilon);
			Obliquity(T, out epsilon, out epsilonNull);

            // determine the mean sidereal time for the given day
			double mst = MeanSiderealTime(jd);
		
			// determine the apparent sidereal time
			return mst + deltaPsi / 15.0 * Math.Cos(epsilon * Deg2Radian) / 240.0;
		}
		...
		public static void Obliquity(double T, out double epsilon, out double epsilonNull)
		{
			double	deltaPsi = 0;		/* nutation of longitude in arc seconds */
			double	deltaEpsilon = 0;	/* nutation of obliquity */
		...
		public static void Nutation(double T, out double deltaPsi, out double deltaEpsilon)
		{
            ...
            deltaPsi = -17.20 * Math.Sin(omega * Deg2Radian) - 1.32 * Math.Sin(2 * L * Deg2Radian)
                - 0.23 * Math.Sin(2 * Lprime * Deg2Radian) + 0.21 * Math.Sin(2 * omega * Deg2Radian);
            deltaEpsilon = 9.20 * Math.Cos(omega * Deg2Radian) + 0.57 * Math.Cos(2 * L * Deg2Radian)
		...
		public static double[] RiseTranSet(double L, double phi, double h0, double JD, double[] A, double[] D)
		{
			...
			if ( Math.Abs( -Math.Sin(phi * Deg2Radian) * Math.Sin(D[1] * Deg2Radian) / Math.Cos(phi * Deg2Radian) * Math.Cos(D[1] * Deg2Radian) ) > 1 )
				throw new AAException("The body is above or below the horizon all day and thus has no rising or setting time.");
